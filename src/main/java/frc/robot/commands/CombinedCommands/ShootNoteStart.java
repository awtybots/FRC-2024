...
public class ShootNoteStart {
  ...
  public static Command run(
      Intake sIntake, Arm sArm, ArmElevator sArmElevator, Wrist sWrist, Flywheel sFlywheel) {
    return Commands.run(
        () -> {
          ShootClosePosition.run(sArm, sArmElevator, sWrist);
          IntakeShooterControls.intakeShooterDrive(sIntake, sFlywheel, () -> 0, () -> 1, () -> true)
              .withTimeout(3);
          StowPosition.run(sArm, sArmElevator, sWrist);
        },
        sIntake,
        sArm,
        sArmElevator,
